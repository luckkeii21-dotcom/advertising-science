...
### GA-065 · Google's "Limited by budget" label: the STATUS is real, the operator's causal reading came from a sales rep, and the check he ran to confirm it does not support his conclusion
Tier: T3 · Status: contested
...
Sources: Blue Sense Digital, 2025-03-05 (Limited By Budget Explained)
Last touched: 2026-08-27

### GA-066 · Google has its own suggestion-versus-control split, and it is called Optimized Targeting. It is ON by default and it expands delivery past the audience you chose
Tier: T1 · Status: active
...
### GA-067 · Google's top-of-funnel ceiling arrives sooner for a single suburb than for a national catalogue, and the operator's evidence for it is nothing
Tier: T4 · Status: active
...
Sources: Blue Sense Digital, 2025-02-06 (Why Meta Ads Is Beating Google Ads In 2025)
Last touched: 2026-08-27

### GA-068 · Google's August 2026 Demand Gen drop ships messaging-app conversations, travel personalisation and generally-available multimodal video, and carries a vendor 30% conversion figure that sits directly against our own measured Demand Gen holdout
Tier: T1 · Status: active
Three product changes, from Google's own Ads & Commerce blog on 2026-08-27.
**One, messaging-app conversations.** Google is "testing new ways for viewers to start conversations with brands on messaging apps directly from Demand Gen ads on YouTube". Stated as a test, not a launch. This is the first Demand Gen surface on file that produces a conversation rather than a click, which makes it the first version of the format that maps onto a lead-gen funnel at all.
**Two, travel and hospitality personalisation.** Surfacing "local activities, events, and real-time offers" and personalising hotel ads by audience. Vertical-specific, no read-across for our book.
**Three, multimodal video creation in Asset Studio is now generally available**, previously in testing. It produces "horizontal and vertical assets in one workflow" from the storyboarding stage, so one build covers every Demand Gen inventory type. This is the only one of the three that changes production economics rather than delivery.
**The number, and why it must be quoted with its label attached.** Google states a "30% average increase in conversions or conversion value" for campaigns using Demand Gen improvements, sourced on the page as "Google Internal Data, Global, H2 2025". **That is a vendor aggregate over self-selected adopters with no control group, no holdout, and no methodology published. It is not a lift measurement and must never be repeated to a client as one.**
**Read it against what this codex already measured.** [[Google Auction & Smart Bidding#GA-055|GA-055]] holds a 21-day Demand Gen geo holdout that produced no statistically relevant NEW-customer lift, 85 orders against the 166 required, while returning-customer orders rose 6.8%; [[Attribution & Incrementality#AT-072|AT-072]] holds the same test as the new-versus-returning split rule and [[Google PMax & Shopping#GP-040|GP-040]] holds the channel verdict. [[Google Auction & Smart Bidding#GA-056|GA-056]] holds the same operator's reading that Demand Gen harvests users already in the funnel. **The two are not strictly in contradiction, because they measure different things: Google is reporting attributed conversions across adopters, our banked test is reporting incremental new customers on one advertiser. They are, though, the exact pair a rep will use to argue for budget, so the honest position is that Google's 30% and our null result can both be true at once, and only the null one was measured against a counterfactual.** That same operator time-stamps his own verdict and tells viewers to check the publication date because Google keeps changing the product, which is the correct posture toward this entry too.
**Forward note, 2026-09-25.** This is one instalment of a **monthly** series, which this codex did not know until the September drop linked Google's Demand Gen Drops Hub. The September 2026 drop is at [[Google Auction & Smart Bidding#GA-081|GA-081]], and two earlier ones read the same day are at [[Google Auction & Smart Bidding#GA-083|GA-083]] (May, where Maps inventory and automotive product feeds actually landed) and [[Google Auction & Smart Bidding#GA-084|GA-084]] (July, where Google says tROAS had been overly cautious early). **Backlog closed 2026-09-26. All thirteen instalments are now read**, and the full census of what every headline figure in the series rests on is at [[Google Auction & Smart Bidding#GA-088|GA-088]]. The 30% recorded here is one of seventeen such figures, and its “conversions or conversion value” phrasing also carries the November 2025 drop, which makes it a house construction rather than a one-off. The running note on how to read them is at GA-081.
...
### GA-069 · Google is REMOVING campaign-level language targeting from Search and from the Search half of Performance Max in September 2026, and the language of your ad becomes the matching signal
Tier: T1, off Google's own help documentation · Status: active
The most operationally significant platform item found in a month, and it lands next month.
**Google's help page states it plainly:** "Starting in September 2026, the way language targeting works is changing for Search campaigns and the Search Network portion of Performance Max campaigns." For Search: **"The campaign-level language targeting setting will be removed. Search ads will automatically match based on the language of your ads."** Google adds that no advertiser action is required.
**Scope, stated precisely, because the halves differ.** Search campaigns and AI Max for Search lose the setting outright. Performance Max keeps its campaign-level language setting for YouTube, Display, Discover and Gmail, and loses it **only for the Search Network portion**. So a single PMax campaign will run one channel on the ad's language and the rest on the old control.
**Existing criteria are not deleted, they stop working.** Language criteria already on a campaign can stay and simply no longer affect targeting.
**Why this belongs beside law 1a rather than in a settings changelog.** Language was a CONTROL on Google, in the sense law 1a uses the word: a hard boundary the platform would not cross, of the same family as location. Google is converting it into an inference drawn from the creative plus signals about what the user understands. **That is the Google-side instance of "creative is the targeting", arriving as an announced product change rather than as a thing operators noticed.** It is the second cross-platform parallel found in a month, after Optimized Targeting at [[Google Auction & Smart Bidding#GA-066|GA-066]], and it is stronger than GA-066 because this one is documented and dated. Read it beside [[Google PMax & Shopping#GP-042|GP-042]], which found the same pattern on the PMax new-customer toggle.
**What it means for a language-split account, and this is where to be careful.** Reporting on the change says Google's position is that accounts separating English, Spanish or other languages into their own campaigns can keep that structure, because the creative and the landing page carry the matching signal instead. **That reads as reassurance and it is a real loss of control: the campaign no longer bounds who sees which language, so the ad copy and the destination page are now the only things doing that job.** Any account of ours built to keep Spanish creative in front of Spanish speakers has to have that guaranteed by the creative from September, not by the setting.
**One detail read on a secondary source and NOT confirmed on Google's own page:** that adding or updating language criteria on a Search campaign will return `ContextError.OPERATION_NOT_PERMITTED_FOR_CONTEXT` through the API. Treat that as reported, not verified, until it is read on Google's developer documentation.
**Read quality, stated honestly.** The Google Ads Developer Blog carries the announcement post, dated 13 August 2026 and tagged `google_ads_api` and `google_ads_scripts`, but its body did not render to two separate fetches, so the title and date come from the blog index and the substance comes from the help centre page plus trade coverage. The help-centre quotations above were read directly.
Sources: Google Ads Help, About language targeting (answer 1722078), read 2026-08-31; Google Ads Developer Blog index entry "Google Ads language targeting changes starting September 2026", 2026-08-13; Search Engine Land, Google Ads is removing language targeting from Search campaigns, August 2026
Last touched: 2026-08-31

### GA-070 · There is no correct keyword count; the test is whether one ad group's keywords can be served by ONE intent, ONE set of ads and ONE landing page, and single-keyword ad groups are no longer the default answer
Tier: T4 · Status: active
...
### GA-073 · Google's 2026-09-10 measurement drop moves Data Manager into Analytics and DV360, makes the Data Manager API universal on the IAB ECAPI standard, and adds a Data Strength Uplift metric that is a Google-computed counterfactual, not a measurement
Tier: T1 · Status: active
Google's Ads & Commerce blog, 2026-09-10, posted by Nipoon Malhotra, VP Ads Analytics, Insights and Measurement. Five product facts on the first-party-data side, read in full from the post body.
**One, Data Manager is being integrated directly into Google Analytics and Display & Video 360**, so first-party data is managed and activated across the three tools from one place.
**Two, enhanced conversions is launching inside Analytics and DV360**, previously an Ads-side feature. Our own [[Google Auction & Smart Bidding#GA-031|GA-031]] lane already puts enhanced conversions in the first 90 days of any account, so this widens where the same setup applies rather than changing the instruction.
**Three, the Data Manager API is now universal and is built on the IAB Tech Lab Event and Conversions API (ECAPI) standard**, described as "a unified, secure setup to connect, manage, and activate your audiences and measurement across major ad platforms." A cross-platform conversions-API standard is the first thing on file that would let one server-side pipeline feed more than one ad platform.
**Four, built-in diagnostics in Data Manager** that "automatically identify and address data issues before they impact campaign performance." No detail on what it detects.
**Five, a new Data Strength Uplift metric in Google Ads**, which "calculates the additional conversions recovered by your first-party data setup."
**The guard on the fifth item is the part to operate on.** A number Google computes for how many conversions Google's own product recovered for you is a vendor counterfactual with no holdout and no published method. It has the same standing as Opportunity Score and Performance Planner, which the law layer already files at law 4c as vendor output and never as measurement. Expect it in a rep's deck. Do not repeat it to a client as a lift.
**The four figures in the post, with the provenance Google itself printed.** 26% average increase in incremental ROAS for advertisers who connect offline and app data to Data Manager, sourced "Google Data, Global, Measurement, April 2025 - April 2026. Based on Search campaigns bidding to conversion value." 11% average increase in Search conversions from enhanced conversions versus standard conversion imports, sourced "Google Internal Data, Global, Ads Log Conversion Data, standard conversion imports vs. Enhanced Conversions on imported conversions, January 1, 2026 to January 14, 2026", a 14-day window. 14% average conversion uplift from Google tag gateway, sourced "Google Internal Data, Global, Finance, July -Dec 2024 vs Jan-June 2025", which is a finance-department half-year comparison rather than a test. Over 20% uplift for Demand Gen campaigns, sourced "Google Data, Global, Performance, June 3, 2026, - Jun 17, 2026", a 14-day window.
**Every one of the four is a vendor aggregate over self-selected adopters. Not one names a control group or a holdout.** This is the second instance of the same pattern in three weeks, after the 30% Demand Gen figure at [[Google Auction & Smart Bidding#GA-068|GA-068]], and the 20% Demand Gen figure here is aimed at the same product our own 21-day geo holdout found no new-customer lift on ([[Google Auction & Smart Bidding#GA-055|GA-055]], [[Attribution & Incrementality#AT-072|AT-072]]).
**The honest limit.** T1 for what the products ARE, for the surfaces and for the dates, on the [[Meta Delivery & Andromeda#MD-099|MD-099]] precedent of T1-for-existence-and-surface-only. Nothing in it measures anything, no rollout date or eligibility is stated for any of the five, and no operator has been seen using any of them. The measurement half of the same post is at [[Attribution & Incrementality#AT-115|AT-115]].
Sources: Google Ads & Commerce Blog, "Drive profitable growth with new data and measurement tools", 2026-09-10, read in full 2026-09-11
...
**Amendment, 2026-09-26.** The pattern called stable here is now measured across the whole series instead of inferred from three instances. Seventeen headline figures across thirteen instalments are censused at [[Google Auction & Smart Bidding#GA-088|GA-088]]: three carry no footnote at all, three name a real experiment, one cites a third-party retail panel, and the rest are averages over adopters. The “at the same ROI” construction used for this 40% also carries the January 2026 TV-screen figure, so it is house style.
...
**Amendment, 2026-09-26.** The footnote quoted above is not unique to this drop. The October 2025 drop carries the same sentence, the same May to June 2025 window and the same over-fifty-products filter behind a **20%** figure instead of 33%. Both readings of the one study are at [[Google Auction & Smart Bidding#GA-086|GA-086]].
Sources: Google, Demand Gen Drop May 2026, read in full 2026-09-25; October 2025 twin footnote read 2026-09-26
